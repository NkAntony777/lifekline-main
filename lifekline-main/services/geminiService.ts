...
export const generateLiunianAnalysis = async (
  ...
  const userPrompt = `
你是一位精通盲派命理的专家，请严格遵循盲派理论，对指定流年进行分析。核心铁律：流年是应期。其根本吉凶，在于流年的作用是否直接帮助命主完成了原局“体”取“用”的做功意图。大运是重要背景，但分析起点与终点必须紧扣原局。
...
辨体用：原局的 “体”（我及工具：日主、印、比、禄、食伤）是什么？“用”（目标：财、官）是什么？
明做功：原局设计的核心做功方式是什么？即“体”打算通过何种方式（合、冲、制、墓）去获取“用”？

第二步：审视流年的全部“工具”与意图（关键升级）
流年不仅通过其天干地支作用于命局，其地支藏干中蕴藏的力量，若恰好是原局关键的“体”或“用”，则会被激活成为该年显性的重要工具或目标，彰显流年深层次的意图。

直接作用（最基本与明显）：流年天干、地支与原局产生的刑、冲、克、合、害等关系。这直接触动了哪个“宫位”（年/月/日/时）和哪个“十神”（财/官/印/食等），这些宫位和十神的含义与引申含义是什么。这是最核心的应事根据。

藏干透出（精髓）：仔细审视流年地支的藏干。若其中任一藏干的天干（例如：巳中藏丙、戊、庚），在原局中扮演着核心“体”或“用”的角色，则意味着该股强大的、此前潜伏的力量在本年被“授予”命主使用或需要面对。这是判断流年质量高低的核心信号之一。
若为关键“体”：等于流年额外赠予命主一件强大的专属工具，助其取“用”，大吉之兆。
若为关键“用”：等于流年将最重要的目标明确摆上台面，命局能量将聚焦于此，成格者得，破格者失。

第三步：评估大运在此过程中的角色（辅助）
分析在当前大运背景下，流年的上述作用被如何放大、缓冲或扭曲。

【三、输出要求（请严格按此格式，直接给结论）】
请基于以上 “原局为体，流年工具与意图，大运为境” 的逻辑分析，直接给出精准断语，分析过程不要输出太多。

1. 本年定调
用一句话，点明本年对实现原局根本意图而言，是“成”是“败”。
示例（成）：“本年，原局‘食神制杀’之意图，“在${dayun}运[某具体]的背景下，得流年强力引动而成就，为肃清阻力、执掌权柄之年。”
示例（败）：“本年，流年严重破坏原局‘印星化杀’之做功路径，为贵人失势、压力骤增之年。”

2. 事件直断
► **原局根本意图**：命主本欲以[原局之体] [做功方式] [原局之用]。
► **大运环境背景**：当前${dayun}大运，其[作用]使此意图的实现环境[整体偏吉/偏凶/平缓]。
► **流年核心引动（关键）**：**流年${liunian}力量直接介入**，其[干支/透出藏干] [作用关系]于原局[关键体/用]（位于[宫位]）。此举[决定了本年根本吉凶方向]，大运环境起到[放大/缓和/转化]作用。
► **现实应象**：此事将主要应在[具体领域]，表现为[非常具体的事件描述]。
示例（成）：
► 原局意图：命主本欲以时柱癸水食神（体），克制月柱己土七杀（用），以求职权。
► 当前大运态势：当前甲寅大运，干支木气生助食神，使“制杀”的“体”变得强而有力，意图实现环境得天时之助。
► 流年干预：甲辰年，流年天干“甲木”合克原局“己土七杀”，同时**地支“辰”地支藏干癸水，透出“癸水食神”**。流年意图在于一举助成“食神制杀”之局。
► 现实应象：此事将主要应在事业竞争领域，表现为凭借过硬技术或方案（食神），在考核、竞标中战胜对手（制杀），获得职位或项目授权。

示例（败）：
► 原局意图：命主本欲以日支卯木禄神（体），合克年支戌土财库（用），以求财。
► 当前大运态势：当前癸亥大运，水旺木浮，使“禄神”之根虚浮不稳，取财意图的实现环境根基不牢。
► 流年干预：酉年，流年地支“酉金”直接冲克原局作为工具和身体的“卯木禄神”。此举彻底破坏“禄神取财”之根基。
► 现实应象：此事将主要应在财运与健康领域，表现为因身体突发伤病或决策失误，导致无法继续经营某个项目，从而破财；或与合作方（酉为劫财）决裂损财。

3. 关键月份
指出最相关、应象最强的1-2个月份。

示例：事态发展的关键窗口在 农历四月（巳月，公历约5月） 与 农历八月（酉月，公历约9月），前者引动贵人机遇，后者关乎利益结算或竞争结果。
